...
var teenStudentsName = studentList
...
var studentNames2 = studentList.Where(s => s.Age > 18)
                              .Select(s => s)
                              .Where(su => su.StandardId > 0)
...
// LINQ multiple filter and select (method sintax)
Console.WriteLine("LINQ multiple filter and select (method sintax)");
var studentNames2A = studentList.Where(s => s.Age > 18 && s.StandardId > 0)
                              .Select(s => s.StudentName);
studentNames2A.ToList().ForEach(x => Console.WriteLine(x));
Console.WriteLine("---");

//// LINQ multiple filter and select (query sintax)
Console.WriteLine("LINQ multiple filter and select (query sintax)");
studentNames2 = from s in studentList
                where s.Age > 18
...
var teenStudentsName3 = studentList
            .Where(s => s.Age > 12 && s.Age < 20)
            .Select(s => new { StudentId = s.StudentId, ImeStudenta = s.StudentName + " Jr." });
...
var teenStudentsName2 = from s in studentList
                        where s.Age > 12 && s.Age < 20
                        select new { StudentId = s.StudentId, ImeStudenta = s.StudentName + " Jr." };
...
var studentsGroupByStandard = studentList.GroupBy(s => s.StandardId)
    .OrderBy(sg => sg.Key)
...
studentsGroupByStandard = from s in studentList
...
var studentiGrupisaniPoStandardu2 = studentList
...
studentiGrupisaniPoStandardu2 = from s in studentList
                                where s.StandardId > 0
                                group s by s.StandardId into sg
                                orderby sg.Key
...
var studentsWithStandard = standardList
    .Join(studentList,
        std => std.StandardId,
        stu => stu.StandardId,
        (std, stu) => new
    ...
    .OrderByDescending(rec => rec.StandardName)
    .ThenByDescending(rez => rez.StudentName);
...
var studentsWithStandardX = from std in standardList
...
studentsWithStandardX.ToList().ForEach(s => Console.WriteLine("{0} is in {1}", s.StudentName, s.StandardName));
Console.WriteLine("---");

//// LINQ left outer join (method sintax)
Console.WriteLine("LINQ left outer join (method sintax)");
var studentsWithStandard2 = standardList
    .GroupJoin(
        studentList,
        std => std.StandardId,
        stu => stu.StandardId,
